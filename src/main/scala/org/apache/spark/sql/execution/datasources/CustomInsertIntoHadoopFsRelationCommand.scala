...
case class CustomInsertIntoHadoopFsRelationCommand(
                                                    outputPath: Path,
  ...
  override def outputColumns: Seq[Attribute] = {
   ...
   val columns = super.outputColumns
  //  columns.zip(attributes).map(cl => cl._1.withDataType(cl._2.dataType))
    columns
  }

   def getMetadataAttributes: Seq[Attribute] = {
//    if(catalogTable.isDefined){
//      val partitionColumnNames = partitionColumns.map(_.name)
//      attributes.filter(dc => !partitionColumnNames.contains(dc.name))
//    }else{
//      outputColumns
//    }
     outputColumns
...
//      catalogTable.get.tracksPartitionsInCatalog
    ...
    val partitionsTrackedByCatalog = false
      ...
      val updatedPartitionPaths =
        FileFormatWriter.write(
          ...
          outputSpec = FileFormatWriter.OutputSpec(
            committerOutputPath.toString, customPartitionLocations, getMetadataAttributes),
...
/**Turning off for Custom Implementation starts**/
...
//      }
      ...
      /**Turining off for Custom Implementation ends**/
...
//      }
  ...
  override protected def withNewChildInternal(
                                               newChild: LogicalPlan): CustomInsertIntoHadoopFsRelationCommand = copy(query = newChild)
}
