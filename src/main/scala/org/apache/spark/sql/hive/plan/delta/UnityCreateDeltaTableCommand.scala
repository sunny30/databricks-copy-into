package org.apache.spark.sql.hive.plan.delta
...
case class UnityCreateDeltaTableCommand(table: CatalogTable,
                                        ...
                                        protocol: Option[Protocol] = None) extends LeafRunnableCommand
  with DeltaLogging {

  override def run(sparkSession: SparkSession): Seq[Row] = {

    log.info("Entering CustomCreateDeltaCommand")

    var table = this.table
    var updateCatalogDone = false
    ...
    val isExternalTable = (table.tableType == CatalogTableType.EXTERNAL)
    ...
    log.info("Loaded External table delta log" + deltaLog.toString)
    ...
    recordDeltaOperation(deltaLog, "delta.ddl.createTable") {
      ...
      if (query.isDefined) {
        ...
        log.info("Inside if query is defined")
        ...
        query.get match {
          case writer: WriteIntoDelta =>
            log.info("Inside WriteIntoDelta")
          ...
          case cmd: RunnableCommand =>
            log.info("Inside RunnableCommand")
        ...
        def createTransactionLogOrVerify(): Unit = {
          ...
          if (noExistingMetadata) {
            ...
            log.info("createTransactionLogOrVerify Txn to be committed ========>")
            //updateCatalog(sparkSession, tableWithLocation, deltaLog.snapshot, txn)
            updateCatalogDone = true
            commitORollBackTxn(sparkSession, txn, tableWithLocation.identifier, Nil, op)
            log.info("createTransactionLogOrVerify Txn committed ========>")
          } else {
            log.info("verifyTableMetadata inside ========>")
        ...
        operation match {
          ...
          case _ =>
            ...
            replaceTable(sparkSession, tableWithLocation, deltaLog.snapshot, txn)
            updateCatalogDone = true
            log.info("Txn to be committed ========>")
            commitORollBackTxn(sparkSession, txn, tableWithLocation.identifier, removes, op)
        }
      }
      ...
      //this is just safe guard this should not occour from CMD-PROC because we dont support CTAS,SAVEAS from CMD-PROC
      if (!updateCatalogDone) {
        log.warn("This is wrong update from HMS, shouln't happen")
        if (tableWithLocation.schema.nonEmpty) {
          //updateCatalog(sparkSession, tableWithLocation, deltaLog.snapshot, txn)
        } else {
         // updateCatalog(sparkSession, tableWithLocation, deltaLog.snapshot, txn, true)
        }
  ...
  private def verifyTableMetadata(
    ...
    if (txn.readVersion > -1) {
      ...
      /*
      *    the reason for commenting out this code LakeHouse we insert few table properties while
      *    creating the table from LakeHouse
      *    any ways these properties are just for persistence in HMS.
      * */
  ...
  private def updateCatalog(
                             ...
                             snapshot: Snapshot,
                             txn: OptimisticTransaction,
                             isProjection: Boolean = false): Unit = {
    val cleaned = cleanupTableDefinition(table, snapshot, isProjection)
    try {
      operation match {
      ...
      case e: Throwable => if (e.getMessage.contains("if path is a directory") && !e.getMessage.contains("Lakehouse rejected operation")) {
        log.info("error not valid, because spark tables validates about directory which is not possible for external as Customer RP missing")
      } else {
        throw e
      }
    }
  ...
  private def replaceTable(
    ...
    spark.sessionState.catalog.dropTable(table.identifier, true, true)
    updateCatalog(spark, table, snapshot, txn)
  }

  def commitORollBackTxn(sparkSession: SparkSession, txn: OptimisticTransaction, ident: TableIdentifier, actions: Seq[Action], operation: DeltaOperations.Operation): Unit = {
    try {
  ...
  private def cleanupTableDefinition(table: CatalogTable, snapshot: Snapshot, isProjection: Boolean = false): CatalogTable = {
    ...
    if (!isProjection) {
      table.copy(
        schema = getHMSSchema(table.schema, table.partitionColumnNames),
      ...
      table.copy(
        schema = getHMSSchema(snapshot.metadata.schema, snapshot.metadata.partitionColumns),
  ...
  // shuffle partition column in the end
  // the source of error is transaction logs
  // Delta logs does not maintain the Hive way of storing schema in Catalog table
  // partition columns should always be in the right most while creating HMS table.
  def getHMSSchema(schema: StructType, partitionColumns: Seq[String]): StructType = {
    val nonPartSchema = schema.filter(p => !partitionColumns.contains(p.name))
    val partSchemaInOrder = partitionColumns.flatMap(p => {
      schema.filter(s => s.name.equalsIgnoreCase(p))
    })
    val result = StructType.apply(nonPartSchema ++ partSchemaInOrder)

    log.info("HMS schema is " + result.sql)
    result
  }
