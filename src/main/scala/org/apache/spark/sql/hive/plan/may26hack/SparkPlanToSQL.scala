...
class SparkPlanToSQL {
  ...
  def toSQL(plan: LogicalPlan): String = plan match {
    ...
    case Project(projectList, child) =>
      ...
      child match {
        ...
        case _                 => s"SELECT $cols\nFROM ${childSQL(child)}"
      }

    // ── Filter / WHERE / HAVING ──────────────────────────────────────────────────
    case Filter(condition, agg: Aggregate) =>
      s"${toSQL(agg)}\nHAVING ${exprToSQL(condition)}"
    ...
    case Aggregate(groupingExprs, aggregateExprs, child) =>
      ...
      else s"\nGROUP BY ${groupingExprs.map(exprToSQL).mkString(", ")}"
      s"SELECT $cols\nFROM ${childSQL(child)}$groupBy"
    ...
    case w: Window =>
      ...
      s"SELECT $allCols\nFROM ${childSQL(w.child)}"

    // ── Join ─────────────────────────────────────────────────────────────────────
    case Join(left, right, joinType, condition, _) =>
      val joinStr = joinTypeToSQL(joinType)
      val on      = condition.map(c => s"\n  ON ${exprToSQL(c)}").getOrElse("")
      s"${childSQL(left)}\n$joinStr JOIN ${childSQL(right)}$on"
    ...
    // ── Limit / Offset ───────────────────────────────────────────────────────────
    case GlobalLimit(le, LocalLimit(_, child)) => s"${toSQL(child)}\nLIMIT ${exprToSQL(le)}"
    case GlobalLimit(le, child)                => s"${toSQL(child)}\nLIMIT ${exprToSQL(le)}"
    case LocalLimit(le, child)                 => s"${toSQL(child)}\nLIMIT ${exprToSQL(le)}"
    case Limit(le, child)                      => s"${toSQL(child)}\nLIMIT ${exprToSQL(le)}"
    case Offset(oe, child)                     => s"${toSQL(child)}\nOFFSET ${exprToSQL(oe)}"
  ...
  private def namedExprToSQL(expr: NamedExpression): String = expr match {
    case Alias(WindowExpression(wf, ws), name) =>
      s"${windowFuncToSQL(wf)} OVER ${windowSpecToSQL(ws)} AS $name"
    case Alias(child, name) => s"${exprToSQL(child)} AS $name"
  ...
  private def windowFuncToSQL(func: Expression): String = func match {
    ...
    case Lag(input, offset, default, _) =>
      val d = default match { case Literal(null, _) => ""; case d => s", ${exprToSQL(d)}" }
