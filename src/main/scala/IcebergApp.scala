...
//    spark.sql(
//      """CREATE TABLE if not exists local.db.logs11 (
...
//    spark.sql("alter table local.db.logs11 add partition field bucket(2,age) as new_p")
//    spark.sql("insert into local.db.logs11 values('a', 'b',1,2), ('a', 'b',2,2)")
  ...
  //  spark.sql("alter table local.db.logs add partition field bucket(2,level) as new_p1")
    ...
    initialData.write.format("iceberg").mode("overwrite").saveAsTable("local.db.logs14")
    initialData.write.format("parquet").mode("overwrite").saveAsTable("local.db.logs14")

    spark.sql("describe formatted local.db.logs13").show()
