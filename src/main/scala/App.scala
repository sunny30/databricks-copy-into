...
//        "file:///Users/sharadsingh/Dev/databricks-copy-into/src/main/resources/jackson-databind-2.6.7.3"+
      ...
      .set("spark.sql.catalog.live","org.apache.spark.sql.hive.catalog.UnityCatalog")
...
//    spark.sql("create table cat.dbicex.tbl(id int, name string)  using iceberg PARTITIONED BY (name)")
//    spark.sql("replace table cat.dbicex.tbl(id int, name string)  using iceberg PARTITIONED BY (truncate(2, name))")
