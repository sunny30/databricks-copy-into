...
//    spark.sql("create table cat.dbx106.tparquet(id string) using parquet")
    ...
    spark.sql("create database cat.dbx113")
    ...
    df2.write.format("delta").saveAsTable("cat.dbx113.ttd")
    spark.sql("CREATE TABLE cat.dbx113.delta_tbl (col1 String) USING delta")
    spark.sql(""" INSERT INTO cat.dbx113.delta_tbl VALUES ('new_value')""".stripMargin)
    df1.write.insertInto("cat.dbx113.delta_tbl")
    spark.sql("select * from cat.dbx113.delta_tbl").show()
    spark.read.table("cat.dbx113.delta_tbl").show()
